...
## zoxide — 学習するディレクトリジャンプ

[zoxide](https://github.com/ajeetdsouza/zoxide) は `cd` の代替ツールです。過去に移動したディレクトリを学習し、ディレクトリ名の断片を入力するだけで目的地にジャンプできます。

### cd との比較

```bash
# 通常の cd: フルパスが必要
cd ~/ghq/github.com/myorg/my-project

# zoxide: 断片だけで移動
z my-project
```

頻繁にアクセスするディレクトリほど優先度が上がります（frecency アルゴリズム）。
...
starship と同じキャッシュパターンを使用しています:

```fish
# config.fish.tmpl
if type -q zoxide
    set -l _cache $_cache_dir/zoxide_init.fish
    if not test -f $_cache; or test (command -s zoxide) -nt $_cache
        mkdir -p $_cache_dir
        zoxide init fish >$_cache
    end
    source $_cache
end
```

zoxide バイナリが更新されたときのみキャッシュを再生成するため、起動時のオーバーヘッドを最小化しています。

### 基本的な使い方

```bash
# 断片でジャンプ（学習済みディレクトリから最も関連性の高い場所へ）
z my-project
...
`zi` は zoxide の学習済みディレクトリを fzf でインタラクティムに選択するコマンドです。

```bash
$ zi
# → fzf が起動、訪問頻度の高いディレクトリ一覧から選択
# → 選択したディレクトリに cd
```
...
| コマンド | 検索対象             | 特徴                                                       |
| -------- | -------------------- | ---------------------------------------------------------- |
| `Alt+C`  | ファイルシステム全体 | fd でディレクトリを列挙（初回でも使える）                  |
| `zi`     | 訪問履歴             | 過去に移動したことがある場所のみ（候補が絞られていて速い） |

普段よく使うディレクトリには `zi`、初めて移動する場所や場所を忘れた場合は `Alt+C`、という使い分けが便利です。

## まとめ

fzf 単体でも便利ですが、fd や bat、zoxide と組み合わせることで真価を発揮します。
...
| zoxide + fzf  | 訪問履歴からのインタラクティブディレクトリジャンプ |
