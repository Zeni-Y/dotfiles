...
|---------|---------------------|-------------|
| シェル | **zsh** | [zsh と sheldon](./05-zsh-and-sheldon) |
| プラグイン管理 | **sheldon**（zsh-defer で遅延読み込み） | [zsh と sheldon](./05-zsh-and-sheldon) |
| プロンプト | **starship** | [モダン CLI ツール](./07-modern-cli-tools) |
| ランタイム管理 | **mise** | [mise によるランタイム管理](./06-mise-runtime-management) |
| エディタ | **Zed**（メイン）/ **vim**（サーバー用） | [Zed エディタ](./14-zed-editor) |
| CLI ツール | **eza**, **fzf**, **fd**, **bat** 等 | [モダン CLI ツール](./07-modern-cli-tools) / [fzf](./08-fzf-and-search) |
| ターミナルマルチプレクサ | 未使用（Zed の統合ターミナルで代用） | — |
...
Zed の設定やキーバインドについては [Zed エディタ](./14-zed-editor) チャプターで詳しく解説しています。
...
**この dotfiles ではマルチプレクサを使っていません。** Zed の統合ターミナルでペイン分割・タブ切り替えができるため、ローカル作業では不要と判断しています。

ただし、**SSH でリモートサーバーに接続する場合は tmux を検討する価値があります**。接続が切れてもセッションが生き続けるため、長時間の処理を安全に実行できます。
...
tmux あり:  PC ──✕── サーバー → セッションは生き続ける
            PC ──再接続──→ tmux attach で復帰
```

tmux の詳細は以下を参照してください:

- [MIT: The Missing Semester — Command-line Environment](https://missing.csail.mit.edu/2020/command-line/)（[日本語訳](https://missing-semester-jp.github.io/2020/command-line/)）— tmux の基本操作を丁寧に解説
- [Zellij 公式](https://zellij.dev/) — tmux に代わるモダンな選択肢
